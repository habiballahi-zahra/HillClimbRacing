...
public class EnviromentGenerator : MonoBehaviour
{
    ...
    [SerializeField, Range(1f, 50f)] private float _xMultiplier = 3f;
    ...
    private void OnValidate()
    {
        ...
        {
            if (i > 0 && i >= _levelLength - 5)
            {
                _lastPos = transform.position + new Vector3(i * _xMultiplier, Mathf.PerlinNoise(0, i * _noiseStep) * 0);
            }
            else
            {
